...
//文档https://www.cnblogs.com/anmutu/p/golang.html
...
type Interface interface {
	PushMessage(buf []byte)
	ConsumeSimple(handleFunc HandleFunc)
	ConsumeSimpleWork(handleFunc HandleFunc)
}

// NewRabbitMQ 创建RabbitMQ结构体实例
func NewRabbitMQ(queueName string, exchange string, key string) *RabbitMQ {
	//fmt.Println(conf.RabbitMQConfig.Uri)
	rabbitMQ := &RabbitMQ{
		QueueName: queueName,
		Exchange:  exchange,
		Key:       key,
	...
	rabbitMQ.failOnErr(err, "rabbitmq 创建连接错误")
	rabbitMQ.channel, err = rabbitMQ.conn.Channel()
	rabbitMQ.failOnErr(err, "rabbitmq 获取Channel失败")
	return rabbitMQ
}
...
func NewRabbitMQSimple(queueName string) *RabbitMQ {
	...
	r := NewRabbitMQ(queueName, "", randSeq(10))
...
func (r *RabbitMQ) PushMessage(buf []byte) {
	//2、发送消息到队列中
	r.channel.Publish(
		r.Exchange,
		r.QueueName,
...
func (r *RabbitMQ) ConsumeSimple(handleFunc HandleFunc) {
	//1、申请队列
...
func (r *RabbitMQ) ConsumeSimpleWork(handleFunc HandleFunc) {
	...
	for i := 0; i < 5; i++ { // 通过协程创建5个消费者
		go func(number int) {
			for d := range msgs {
				...
				//log.Sugar.Infof("Received a message : %s, 消费者编号: %d", d.Body, number)
				//实现我们要处理的逻辑函数
	...
	//log."[*] Waiting for messagees,To exit press CTRL+C")
